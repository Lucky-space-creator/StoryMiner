...
系统能力按模块组织（M1–M15），覆盖「解析 → 向量化 → 知识提取 → 生成增强 → 运营」全链路。
数据库 Schema 版本化迁移脚本见 `story_sql/`（`V1`–`V22`，按序执行）。
...
数据库 Schema 由 ORM `create_all` 驱动；版本化迁移脚本见 `story_sql/`（`V1`–`V22`，按序执行）。
