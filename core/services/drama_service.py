...
class DirectorPrompt:
    ...
    SYSTEM = (
        ...
        "0. prev_chapter_review（上集回顾）：单个字符串，100–200 字，概括上一章的大致内容与剧情发展，"
        "点明关键人物、核心冲突与未解悬念，让本集观众无需回顾即可衔接。仅当给定了【上一章正文】时才输出本字段；若本集为开篇（无上一章）则输出空字符串 \"\"。\n"
        ...
        "2. plot_arrangement（剧情安排）：字符串数组，每条都是一段「可直接作为漫剧动画剧情」的连续叙事，"
        ...
        "每条应像小说正文般连贯可读，能让下游动画生成直接据此还原「人物神态+台词+实时场景变化」的画面。\n"
        ...
        "硬性约束：单集总时长最长不得超过 15 分钟，若按正文规模估算会超限，必须压缩镜头数量或合并场景使总时长落在 15 分钟以内（建议 8–15 分钟区间）。\n"
        "【原文还原与精简规则——最高优先级】\n"
    ...
    def build_user_prompt(cls, d, char_block: str, full_text: str, prev_text: str | None = None) -> str:
        ...
        if prev_text:
            parts.append(f"【上一章正文（请据此生成 prev_chapter_review 上集回顾 100–200 字）】\n{prev_text[:6000]}")
...
async def director_agent_generate(
    ...
    try:
        ...
        # 服务端超时保护：本地 7B 偶发缓慢（可达数分钟），若超过 170s 仍未返回，
        # 主动取消并走兜底，确保 finally 能及时清除 generating 标记，
        # 避免前端刷新后永久停留在「生成中」置灰态（前端超时 180s，略大于此值）。
        try:
            raw = await asyncio.wait_for(adapter.chat(messages, json_mode=True), timeout=170)
