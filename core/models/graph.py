...
整体思路：
    映射 V1 已建好的 story_entity / story_relation / story_relation_type 三张表，
    承载小说知识图谱的实体、关系与关系类型字典，是 M5 抽取与展示的数据载体。

关键点：
    1. 表名严格为 story_entity / story_relation / story_relation_type（V1 已建，本文件仅补 ORM 映射）。
...
class Entity(Base):
    ...
    type: Mapped[str] = mapped_column(String(16), default="character", nullable=False)
